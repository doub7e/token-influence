...
exp_name='Archer2.0-Qwen3-1.7B-Base-InflAdvantage-v2'

adv_estimator=grpo

# kl config (no regularization — ep2-clip01-014 baseline)
...
low_entropy_clip_ratio_low=0.1
low_entropy_clip_ratio_high=0.14
...
# Influence trace output directory
influence_trace_dir="${CKPTS_DIR}/influence_trace"
mkdir -p "${influence_trace_dir}"
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_trace.write_every=10 \
    ...
    +trainer.influence_trace.output_function=log_prob_reward \
    +trainer.influence_trace.accepted_rejected_scope=per_prompt \
    +trainer.influence_trace.module_name_filter='[self_attn.q_proj,self_attn.k_proj,self_attn.v_proj,self_attn.o_proj,mlp.gate_proj,mlp.up_proj,mlp.down_proj]' \
    ...
    +trainer.influence_trace.grad_offload_to_cpu=True \
    +trainer.influence_trace.force_gpu_compute=True \
    +trainer.influence_trace.profile_timing=True \
    +trainer.influence_trace.exclude_self_response=False \
    +trainer.influence_trace.contrastive_agg=advantage \
    +trainer.influence_trace.hessian_source=token \
    +trainer.influence_trace.debug_hessian_similarity=False \
    +trainer.influence_trace.score_normalization=h_inv_norm \
    +trainer.influence_token_weight.enabled=True \
    +trainer.influence_token_weight.mode=direct_no_baseline \
    +trainer.influence_token_weight.apply_epochs='[0,1]' \
    +trainer.influence_token_weight.direct_lambda=0.3 \
    +trainer.influence_token_weight.direct_clamp_min=-2.0 \
    +trainer.influence_token_weight.direct_clamp_max=2.0 \
    +trainer.influence_token_weight.adv_target=advantage \
