#!/usr/bin/env bash
# Qwen3-1.7B-Base + TrackStar threshold mask mode (all_selected scope)
# mask_threshold: only mask tokens with |z-score| > threshold. Resume from mask-v6 step 100.
set -xeuo pipefail
...
exp_name='Archer2.0-Qwen3-1.7B-Base-TrackStar-MaskThreshold-AllSel-v6b'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.mode=mask_threshold \
    +trainer.influence_token_weight.apply_epochs='[0,1]' \
    +trainer.influence_token_weight.threshold=1.0 \
