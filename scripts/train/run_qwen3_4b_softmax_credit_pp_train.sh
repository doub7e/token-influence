#!/usr/bin/env bash
# Qwen3-4B-Base + influence softmax credit assignment
# softmax(z/τ) × T: redistribute advantage proportional to influence
# Total advantage preserved (weights sum to T). Rejected responses flip sign.
set -xeuo pipefail
...
exp_name='Qwen3-4B-SoftmaxCredit-T4-clamp05-mb32-pp'
wandb_name='Qwen3-4B-SoftmaxCredit-T4-clamp05-mb32-pp'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.softmax_temperature=4.0 \
    +trainer.influence_token_weight.weight_clamp_min=0.5 \
    +trainer.influence_token_weight.weight_clamp_max=1.5 \
