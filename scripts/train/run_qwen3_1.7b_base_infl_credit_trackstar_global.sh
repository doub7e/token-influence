#!/usr/bin/env bash
# Qwen3-1.7B-Base + TrackStar credit mode (all_selected scope)
# credit mode: A'_t = A_t + λ*z_t (additive correction, no sign flip needed)
set -xeuo pipefail
...
exp_name='Archer2.0-Qwen3-1.7B-Base-TrackStar-Credit-Global-L01-v5e'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.mode=credit \
    +trainer.influence_token_weight.apply_epochs='[0,1]' \
    +trainer.influence_token_weight.additive_lambda=0.1 \
    +trainer.influence_token_weight.additive_clamp_min=-0.5 \
    +trainer.influence_token_weight.additive_clamp_max=0.5 \
