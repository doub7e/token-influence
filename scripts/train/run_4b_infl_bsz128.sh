#!/usr/bin/env bash
# Test influence trace with bsz=128, mini_bsz=32 (optimized training config).
# Compare: lastmlp-f16 (with hessian) vs no-hessian (identity).
# Resume from checkpoint-300, run 1 step only.
set -xeuo pipefail
...
# Config A: lastmlp-f16 with hessian (inverse)
run_one "infl-bsz128-lastmlp-f16-hessian" \
...
# Config B: lastmlp-f16 without hessian (identity)
run_one "infl-bsz128-lastmlp-f16-nohessian" \
    ++trainer.influence_trace.hessian_mode=identity \
    ++trainer.influence_trace.projection_dim_factor=16 \
    ++trainer.influence_trace.max_hessian_dim=50000 \
    ++trainer.influence_trace.max_modules=1 \
    ++trainer.influence_trace.accepted_rejected_scope=all_selected \
...
echo "===== ALL BSZ128 CONFIGS COMPLETE ====="
