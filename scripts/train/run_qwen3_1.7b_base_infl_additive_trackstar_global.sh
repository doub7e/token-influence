#!/usr/bin/env bash
# Qwen3-1.7B-Base + TrackStar token influence weighting (global_selected scope)
# additive_lambda=0.03, token_unit_norm=True, eval every 10 steps on math500
set -xeuo pipefail
...
exp_name='Archer2.0-Qwen3-1.7B-Base-TrackStar-Global-L003-v3'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_trace.accepted_rejected_scope=global_selected \
    ...
    +trainer.influence_trace.profile_timing=False \
    ...
    +trainer.influence_token_weight.mode=additive \
    +trainer.influence_token_weight.apply_epochs='[0,1]' \
    +trainer.influence_token_weight.additive_lambda=0.03 \
    +trainer.influence_token_weight.additive_clamp_min=-1.0 \
    +trainer.influence_token_weight.additive_clamp_max=3.0 \
