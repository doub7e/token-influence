#!/usr/bin/env bash
# Training: influence-guided masking (mask_threshold, T=1.0, allsel, hessian inverse).
# Resume from checkpoint-300, run to step 400.
set -xeuo pipefail
...
NAME="Qwen3-4B-MaskThresh-T1-allsel-hessian"
WANDB_NAME="Qwen3-4B-MaskThresh-T1-allsel-hessian-v2"
DST="$BASE_DIR/$NAME"

mkdir -p "$DST/influence_trace"
if [ ! -f "$DST/latest_checkpointed_iteration.txt" ]; then
    # First run: copy seed checkpoint and set iteration
