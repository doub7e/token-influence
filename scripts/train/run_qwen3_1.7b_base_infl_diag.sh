#!/usr/bin/env bash
# Diagnostic influence trace run — 1 step only, no weighting, no eval.
# Usage: bash run_qwen3_1.7b_base_infl_diag.sh \
#          +trainer.influence_trace.accepted_rejected_scope=<scope> \
#          +trainer.influence_trace.projection_dim_factor=<factor>
# The experiment name and scope/factor are passed via $@ overrides.
set -xeuo pipefail
...
# exp_name is expected to be overridden via $@
exp_name='Archer2.0-Qwen3-1.7B-Base-Diag-Default'

# Same hyperparams as baseline (ep2-clip01-014)
...
# Will be set by override, but need a default for mkdir
...
python -m dapo.main_dapo \
    ...
    actor_rollout_ref.actor.clip_ratio_high=0.14 \
    actor_rollout_ref.actor.clip_ratio_c=3.0 \
    actor_rollout_ref.model.use_remove_padding=True \
    actor_rollout_ref.actor.use_dynamic_bsz=False \
    actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=${micro_batch_size_per_gpu} \
    +actor_rollout_ref.actor.high_entropy_kl_loss_scale_coef=0.0 \
    +actor_rollout_ref.actor.low_entropy_clip_ratio_low=0.1 \
    +actor_rollout_ref.actor.low_entropy_clip_ratio_high=0.14 \
