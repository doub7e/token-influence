#!/usr/bin/env bash
# Qwen3-4B-Base + influence return-to-go mode (allsel, factor=16, unit norm)
# Return mode: G_t = Σ_{k=t}^T s_k (undiscounted suffix-sum), γ=1
...
exp_name='Qwen3-4B-Return-G099-L003-allsel-v2'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.mode=return \
    +trainer.influence_token_weight.return_gamma=0.99 \
    ...
    +trainer.influence_token_weight.apply_to=all \
