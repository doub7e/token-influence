...
exp_name='Archer2.0-Qwen3-1.7B-Base-InflWeight-Tanh-a05'

adv_estimator=grpo

# kl config (stronger KL for base model)
...
# Influence trace config (f=64, per_prompt)
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.mode=tanh \
    +trainer.influence_token_weight.normalize=zscore \
    +trainer.influence_token_weight.apply_epochs='[1,2]' \
    +trainer.influence_token_weight.tanh_alpha=0.5 \
    +trainer.influence_token_weight.tanh_tau=1.0 \
    +trainer.influence_token_weight.weight_clamp_min=0.5 \
    +trainer.influence_token_weight.weight_clamp_max=2.0 \
