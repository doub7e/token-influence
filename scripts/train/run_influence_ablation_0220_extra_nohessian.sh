...
summary_root="output/influence_ablation_0220_extra_nohessian"
log_dir="${summary_root}/logs"
mkdir -p "${log_dir}"

summary_csv="${summary_root}/experiment_cases.csv"
printf "case_id,mode,scope,hessian_mode,exp_name,log_path,offload_to_cpu,status\n" > "${summary_csv}"
...
run_identity_case() {
  ...
  while true; do
    ...
    export INFLUENCE_HESSIAN_MODE=identity
    export INFLUENCE_GRAD_OFFLOAD_TO_CPU="${offload}"

    : > "${log_path}"
    echo "[RUN] case=${case_id} mode=${mode} scope=${scope} hessian=identity offload=${offload} exp=${exp_name}" | tee -a "${log_path}"
    ...
    if [ "${rc}" -eq 0 ]; then
      printf "%s,%s,%s,%s,%s,%s,%s,%s\n" "${case_id}" "${mode}" "${scope}" "identity" "${exp_name}" "${log_path}" "${offload}" "ok" >> "${summary_csv}"
      return 0
    fi
    ...
    printf "%s,%s,%s,%s,%s,%s,%s,%s\n" "${case_id}" "${mode}" "${scope}" "identity" "${exp_name}" "${log_path}" "${offload}" "failed" >> "${summary_csv}"
    return "${rc}"
  done
}

run_identity_case "6" "log_prob_advantage" "per_prompt" "archer-infl-abl-6-logpadv-prompt-f128-identity-0220"
run_identity_case "7" "log_prob_advantage" "all_selected" "archer-infl-abl-7-logpadv-all-f128-identity-0220"
...
root = Path("output/influence_ablation_0220_extra_nohessian")
