#!/usr/bin/env bash
# Run 2 influence trace configs WITHOUT token_unit_norm for comparison.
# lastmlp-f16 and lmhead-f128, same as before but token_unit_norm=False.
set -xeuo pipefail
...
# Common Hydra args (array) — token_unit_norm=False
COMMON_ARGS=(
    ...
    +trainer.influence_trace.token_unit_norm=False
)
...
run_one() {
    ...
    rm -rf "$DST/global_step_300" "$DST/global_step_301" "$DST/global_step_302"
}

# Config A: last MLP, factor=16, max_hessian_dim=50000, NO unit norm
run_one "infl-diag-lastmlp-f16-no-unitnorm" \
...
# Config B: lm_head, factor=128, NO unit norm
run_one "infl-diag-lmhead-f128-no-unitnorm" \
    ++trainer.influence_trace.projection_dim_factor=128 \
    ++trainer.influence_trace.max_hessian_dim=-1 \
    ++trainer.influence_trace.max_modules=1 \
    ++trainer.influence_trace.accepted_rejected_scope=all_selected \
    "++trainer.influence_trace.module_name_filter=[lm_head]"

echo "===== ALL NO-UNITNORM CONFIGS COMPLETE ====="
