#!/usr/bin/env bash
# Qwen3-1.7B-Base + TrackStar soft mask mode (all_selected scope)
# mask_soft: wrong-direction tokens get weight=0.5 instead of 0. Resume from mask-v6 step 100.
set -xeuo pipefail
...
exp_name='Archer2.0-Qwen3-1.7B-Base-TrackStar-MaskSoft-AllSel-v6b'
...
python -m dapo.main_dapo \
    ...
    +trainer.influence_token_weight.mode=mask_soft \
    +trainer.influence_token_weight.apply_epochs='[0,1]' \
    +trainer.influence_token_weight.weight_clamp_min=0.5 \
